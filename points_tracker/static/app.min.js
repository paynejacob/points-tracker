...
self.playAudio=function(id,length){console.log('called');$http.post('/play/'+id,{}).error(function(error){$log.error('audio failed to play: '+error);self.playing='error';}).success(function(audio){$log.log('played');self.playing=id;$log.log(length);$timeout(function(){self.playing=0;},length*1000+500);});};self.getAudioList=function(){$http({url:'/files/'+self.searchQuery,method:'GET',params:{limit:10000}}).error(function(error){$log.error('Fetch audio list failed: '+error);}).success(function(audio){self.audio=audio;});};angular.element(document).ready(function(){self.getAudioList();});}})();(function(){'use strict';angular.module('points_tracker.controllers').controller('DashboardCtrl',DashboardCtrl)
...
$log.info('onBeforeUploadItem',item);};$scope.uploader.uploadAll();$scope.uploader.onCompleteAll=function(value){$state.go('audio-list');$log.info("audio file uploaded",value);toaster.pop('success',"Your file has been added");};}
